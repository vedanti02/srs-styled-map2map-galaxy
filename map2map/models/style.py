...
class ConvStyled3d(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # END HERE
        # Grouped conv: out_channels_total = N * Cout, so bias must match.
        b = self.bias.repeat(N) if self.bias is not None else None
        x = self.conv(x, w, bias=b, stride=self.stride, groups=N)
        _, _, *DHWout = x.shape
